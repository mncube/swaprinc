...
[LearnPCA Step-by-Step PCA vignette](https://cran.r-project.org/web/packages/LearnPCA/vignettes/Vig_03_Step_By_Step_PCA.pdf) before being passed to stats::prcomp. The miss_handler parameter, set to 'omit', 
ensures that only complete cases are included by subsetting the data frame rows 
with stats::complete.cases.

As a recommendation, use broom and broom.mixed to summarize model results to avoid 
lengthy summaries.
